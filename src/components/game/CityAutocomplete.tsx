  ...
  return (
    <div ref={containerRef} className="relative">
      <input
        ...
        className={
          className ??
          "mt-1 w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm disabled:cursor-not-allowed disabled:bg-slate-100 disabled:text-slate-400 dark:border-slate-600 dark:bg-slate-800 dark:disabled:bg-slate-900 dark:disabled:text-slate-500"
        }
      ...
      {!disabled && value.trim() !== "" && (
        <span
          className={`pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-sm ${
            isValid ? "text-emerald-600 dark:text-emerald-400" : "text-slate-300 dark:text-slate-600"
          }`}
          aria-hidden
        >
          {isValid ? "✓" : ""}
      ...
      {showMenu && (
        <ul
          ...
          role="listbox"
          className="absolute left-0 top-full z-20 mt-1 max-h-60 w-full overflow-y-auto rounded-xl border border-slate-200 bg-white p-1 shadow-lg dark:border-slate-700 dark:bg-slate-800"
        >
